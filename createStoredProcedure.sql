...
BEGIN
	...
	START TRANSACTION;
		...
		SELECT reservationId,roomId,customer,startDate,endDate,numOfDays,totalCost,keyReturned,paid,canceled,updatedOn
		FROM reservation 
		WHERE DATE(updatedOn) <= cutoffDate; 
       		
		-- ARCHIVE ROOMSERVICE TABLE
		INSERT INTO archive_roomservice(taskId,task,roomId,reservationId,time,cost,updatedOn)
		SELECT taskId,task,roomId,reservationId,time,cost,updatedOn
		FROM roomservice
		WHERE DATE(updatedOn) <= cutoffDate;
